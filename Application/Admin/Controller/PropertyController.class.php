...
class PropertyController extends AdminController {
    ...
    public function index(){
        ...
        $list = M($Property)->join("INNER JOIN {$fix}{$Property_type} ON {$fix}{$Property}.type = {$fix}{$Property_type}.id")->field("{$fix}{$Property}.*, {$fix}{$Property_type}.type")->select();
		//print_r($list);
        //int_to_string($list);
        // 记录当前列表页的cookie
        Cookie('__forward__',$_SERVER['REQUEST_URI']);
        $this->assign('Property','active');
    ...
    public function add(){
        //获取所有的模型
        $type_list = $this->lists('Property_type','' ,'' ,'id, type');
    ...
    public function edit(){
        ...
        /*获取一条记录的详细数据*/
        $Model = M('Property');
        //$Good_type = M('Property_type')->field('id, type')->select();
        ...
        //$this->assign('Property_type', $Good_type);
        $this->assign('type_list', $type_list);
    ...
    public function del(){
        ...
        $res = M('Property')->where(array('id'=>array('in',$ids)))->delete();
        if(!$res){
    ...
    public function update(){
        $res = D('Property')->update();
        if(!$res){
            $this->error(D('Property')->getError());
        }else{
