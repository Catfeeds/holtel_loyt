...
class PropertyTypeController extends AdminController {
    ...
    public function type(){
    	//echo number_format(1e-7,7);die();
    	$list = $this->lists('Property_type');
    	int_to_string($list);
    	// 记录当前列表页的cookie
    	Cookie('__forward__',$_SERVER['REQUEST_URI']);
    	$this->assign('Property_type','active');
    	$this->assign('_list', $list);
    	//print_r($list);
    	$this->meta_title = '物品登记';
    	$this->display();
    ...
    public function edit(){
        ...
        /*获取一条记录的详细数据*/
        $Good_type = M('Property_type')->where(array('id'=>$id))->find();
        if(!$Good_type){
            $this->error(M('Property_type')->getError());
        }
    ...
    public function del(){
        ...
        M('Property')->where(array('type' => array('in' , $ids)))->delete();
        /*if($Good_list){
        ...
        $res = M('Property_type')->where(array('id'=>array('in',$ids)))->delete();
        if(!$res){
            $this->error(M('Property_type')->getError());
        }else{
            $this->success('删除成功！');
    ...
    public function update(){
       // $res = M('Good_type')->where(array('id'=>I('id')))->save(array('type' => I('type'), 'add_time' => I('add_time')));
    	$res = D('Property_type')->update();
       // echo M('Good_type')->getLastSql();
    	if(!$res){
    		$this->error(D('Property_type')->getError());
    	}else{
